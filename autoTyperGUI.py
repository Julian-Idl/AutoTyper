...
def create_about_window():
	...
	lbl_developer = tk.Label(text="Developer - Julian-Idl", master=about_window, font='Helvetica 10')
	lbl_developer.grid(row=1, column=0,  pady=5, padx=10)

	lbl_twitter = tk.Label(text="Follow - https://twitter.com/JulianDev_007", master=about_window, font='Helvetica 10', fg="blue", cursor="hand2")
	lbl_twitter.grid(row=2, column=0, pady=5, padx=10)
	lbl_twitter.bind("<Button-1>", lambda e: callback("https://twitter.com/JulianDev_007"))

	lbl_github = tk.Label(text="Github - https://github.com/Julian-Idl", master=about_window, font='Helvetica 10', fg="blue", cursor="hand2")
	lbl_github.grid(row=3, column=0, pady=5, padx=10,)
	lbl_github.bind("<Button-1>", lambda e: callback("https://github.com/Julian-Idl"))

	lbl_youtube= tk.Label(text="Instagram - https://www.instagram.com/julian_idl/", master=about_window, font='Helvetica 10', fg="blue", cursor="hand2")
	lbl_youtube.grid(row=4, column=0, pady=10, padx=10,)
	lbl_youtube.bind("<Button-1>", lambda e: callback("https://www.instagram.com/julian_idl/"))

	buy_coffee = tk.Button(text="Portfolio", master=about_window)
	buy_coffee.grid(row=5,column=0, padx=10, pady=(10,20))
	buy_coffee.bind("<Button-1>", lambda e: callback("https://julian-idl.codes/"))
